...
function resolveKey(key: string): string {
  if (!localRoot) {
    throw new Error("Local driver not initialised. Call initLocalDriver(config) at boot.");
  }
...
/**
 * Resolve the on-disk root from config and register the driver. Called
 * once at boot from `modules/file/index.ts`.
 */
export function initLocalDriver(localRootPath: string): void {
  localRoot = isAbsolute(localRootPath) ? localRootPath : resolve(ROOT_DIR, localRootPath);
