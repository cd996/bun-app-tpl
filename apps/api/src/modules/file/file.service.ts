...
let gcMode: "async" | "sync" = "async";

/** Boot-time hook: tell the service which GC mode to run in. */
export function setGcMode(mode: "async" | "sync"): void {
  gcMode = mode;
}
 ...
 */
export async function uploadAndReference(db: AppDatabase, input: UploadInput): Promise<UploadResult> {
  const { file, ownerType, ownerId, uploadedBy } = input;

  if (!isWithinFileSize(file.size)) {
  ...
  const existing = await db.select({ value: count() })
    ...
    .get();
  if ((existing?.value ?? 0) >= MAX_ATTACHMENTS_PER_RESOURCE) {
    throw new AppError(
      `Maximum attachments per resource reached (${MAX_ATTACHMENTS_PER_RESOURCE})`,
      400,
      "LIMIT_EXCEEDED",
    );
  }
 ...
 */
export async function releaseReference(db: AppDatabase, input: ReleaseReferenceInput): Promise<void> {
...
export async function releaseAllByOwner(
  ...
  const refs = await db.select({ id: fileReferences.id })
    ...
    .all();
  for (const r of refs) {
    await releaseReference(db, { referenceId: r.id });
  }
}
...
let presignEnabled = true;
let presignTtlSeconds = 300;

/** Boot-time hook. */
export function setPresignConfig(input: { enabled: boolean; ttlSeconds: number }): void {
  presignEnabled = input.enabled;
  presignTtlSeconds = input.ttlSeconds;
}
...
export async function buildDownloadResponse(
  ...
  const inlineSafe = opts.inline && (
    (mt.startsWith("image/") && mt !== "image/svg+xml")
    || (mt.startsWith("text/") && mt !== "text/html")
    || mt === "application/pdf"
    || mt === "application/json"
    || mt === "application/xml"
  );
  const contentType = inlineSafe ? mt : "application/octet-stream";
  const disposition = `${inlineSafe ? "inline" : "attachment"}; filename="${encodeURIComponent(ref.filename)}"`;

  const driver = getActiveDriver();

  if (presignEnabled && driver.name === file.storageDriver && driver.presignDownload) {
    const url = await driver.presignDownload(file.storageKey, {
      expiresSeconds: presignTtlSeconds,
      filename: ref.filename,
      inline: inlineSafe,
      contentType,
    });
