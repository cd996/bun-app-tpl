...
beforeEach(async () => {
  ...
  setGcMode("sync");
  setPresignConfig({ enabled: false, ttlSeconds: 300 });
...
describe("uploadAndReference", () => {
  test("creates a files row + a file_references row on first upload", async () => {
    ...
    const result = await uploadAndReference(db, {
  ...
  test("dedupes identical content across owners — one files row, two refs", async () => {
    const userId = await seedUser();
    const first = await uploadAndReference(db, {
      file: pngFile("same.txt", "deduped body"),
      ownerType: "item_attachment",
      ownerId: "item-a",
      uploadedBy: userId,
    });
    const second = await uploadAndReference(db, {
  ...
  test("UNIQUE(owner_type, owner_id, file_id) rejects a duplicate reference on the same owner", async () => {
    const userId = await seedUser();
    await uploadAndReference(db, {
      file: pngFile("a.txt", "body"),
      ownerType: "item_attachment",
      ownerId: "item-x",
      uploadedBy: userId,
    });
    await expect(uploadAndReference(db, {
...
describe("releaseReference (sync GC)", () => {
  test("releasing the last reference deletes both the blob and the files row", async () => {
    ...
    await releaseReference(db, { referenceId: first.reference.id });
  ...
  test("releasing one of two references keeps the blob alive", async () => {
    const userId = await seedUser();
    const a = await uploadAndReference(db, {
    ...
    await releaseReference(db, { referenceId: a.reference.id });
...
describe("releaseAllByOwner", () => {
  test("drops every reference for an owner; blobs reclaimed when refcount drains", async () => {
    ...
    const b = await uploadAndReference(db, {
    ...
    await releaseAllByOwner(db, "item_attachment", "owner-1");
...
describe("buildDownloadResponse", () => {
  test("streams body when presign is disabled (local driver)", async () => {
    const userId = await seedUser();
    const up = await uploadAndReference(db, {
      file: pngFile("hello.txt", "stream me"),
      ownerType: "item_attachment",
      ownerId: "x",
      uploadedBy: userId,
    });
    const resp = await buildDownloadResponse(up.file, up.reference, { inline: false });
  ...
  test("forces application/octet-stream on inline downloads of script-bearing types", async () => {
    ...
    // forces octet-stream.
    const userId = await seedUser();
    const realUpload = await uploadAndReference(db, {
    ...
    const resp = await buildDownloadResponse(svgFile, realUpload.reference, { inline: true });
  ...
  test("302s to a signed URL when presign is enabled + driver supports it", async () => {
    ...
    setPresignConfig({ enabled: true, ttlSeconds: 60 });
    ...
    const resp = await buildDownloadResponse(fileRow, refRow, { inline: true });
