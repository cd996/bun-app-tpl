...
//
// In production we use the `__Host-` prefix: browsers refuse to honour the
// cookie unless it is Secure + Path=/ + no Domain attribute, blocking
// subdomain-takeover style fixation. In development (plain HTTP) the
// prefix is incompatible with the missing Secure flag, so we fall back to
// the plain name. A single helper hides the swap.
const OAUTH_STATE_COOKIE_PROD = "__Host-oauth_state";
...
export function authRoutes() {
  ...
  router.get("/account/auth/login", async (c) => {
    ...
    const { state, codeChallenge } = await createPkceChallenge(redirectUri);
  ...
  router.get("/account/auth/callback", async (c) => {
    ...
    if (totpEnabled) {
      ...
      setCookie(c, TOTP_PENDING_COOKIE, challengeId, {
        httpOnly: true,
        secure: config.NODE_ENV === "production",
        sameSite: "Lax",
        path: "/",
  ...
  router.post("/account/auth/logout", async (c) => {
    ...
    if (sessionId) {
      ...
      if (result) {
        ...
        // Best-effort OAuth token revocation. openid-client picks the
  ...
  router.post("/account/auth/totp/verify", async (c) => {
    ...
    const challengeId = getCookie(c, TOTP_PENDING_COOKIE);
    if (!challengeId) {
    ...
    if (!challenge) {
      deleteCookie(c, TOTP_PENDING_COOKIE, { path: "/" });
    ...
    const lockState = isTotpUserLocked(challenge.userId);
    if (lockState.locked) {
      deleteCookie(c, TOTP_PENDING_COOKIE, { path: "/" });
    ...
    if (!ok) {
      ...
      const stateAfter = isTotpUserLocked(challenge.userId);
      ...
      setCookie(c, TOTP_PENDING_COOKIE, newId, {
        httpOnly: true,
        secure: config.NODE_ENV === "production",
        sameSite: "Lax",
        path: "/",
