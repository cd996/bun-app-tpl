...
function buildAs(oauthConfig: OAuthConfig, appConfig: Config): CachedAs {
  ...
  // is set on the AuthorizationServer object. Opt in only when every
  // endpoint scheme is http and we are not in production — production
  // configs use https across the board.
  const allHttp = [oauthConfig.tokenUrl, oauthConfig.authorizeUrl, oauthConfig.userinfoUrl, issuer]
    .every(u => u.startsWith("http://"));
  const insecure = allHttp && appConfig.NODE_ENV !== "production";
