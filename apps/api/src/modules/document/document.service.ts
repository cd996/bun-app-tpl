...
export interface DocumentRow {
  ...
  readonly parentId: string | null; // sibling short_id
...
async function getParentShortId(db: AppDatabase, parentItemId: string | null): Promise<string | null> {
  if (!parentItemId)
    return null;
  ...
  return await db.transaction(async (tx): Promise<DocumentRow | VersionConflict | undefined> => {
    ...
    if (input.expectedVersion !== undefined && res.rowsAffected === 0) {
      ...
      if (current && current.version !== input.expectedVersion) {
        return { conflict: true, current: await composeDocument(tx as unknown as AppDatabase, current) };
      }
      return undefined;
    }
    ...
    return await composeDocument(tx as unknown as AppDatabase, refreshed);
  });
}
...
export async function getDocumentPermission(
  ...
  if (!item)
    return null;
  // Direct creator is intrinsic editor.
  ...
  return view.allowed ? "viewer" : null;
}
...
export async function addDocumentShare(db: AppDatabase, input: AddShareInput) {
  const item = await getItemByShortId(db, input.documentId);
  if (!item)
    throw new Error("Document not found");
  const now = new Date().toISOString();
  const id = nanoid();

  // Upsert by replacing any existing tuple on (item, X, viewer|editor,
  // <target>) — sharing acts as "set the role" rather than "add another
  // grant", so re-sharing with a different permission updates the row.
  await db.transaction(async (tx) => {
    await tx.delete(relationTuples).where(and(
      eq(relationTuples.namespace, "item"),
      eq(relationTuples.objectId, item.id),
    ...
    await tx.insert(relationTuples).values({
      ...
      subjectNamespace: input.targetType,
      subjectId: input.targetId,
      subjectRelation: input.targetType === "group" ? "member" : null,
...
export async function removeDocumentShare(db: AppDatabase, tupleId: string): Promise<void> {
...
// Re-export the parent-resolution helper for routes that need internal-id lookups.
