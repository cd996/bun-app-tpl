 ...
 * quirk that surfaces as `SQLITE_CORRUPT` on the next cold open. See
 * [`docs/audit/2026-05-13-libsql-corrupt-on-restart.md`](../../../../../docs/audit/2026-05-13-libsql-corrupt-on-restart.md)
 * for the bisection. Until the upstream behaviour is fixed, the SPA is
 * expected to detach a comment's attachments first (the sub-type
 * routes expose `DELETE /comments/:cid/attachments/:aid`); anything
 * left behind becomes orphan `file_references` rows for a future GC
 * sweep extension to reclaim.
 */
