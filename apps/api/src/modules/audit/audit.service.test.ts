...
import { __resetAuditLoggerForTests, audit, getAuditEventById, listAuditEvents, setAuditLogger } from "./audit.service";
...
describe("audit() — console.error fallback", () => {
  test("when no logger has been set, audit() failures fall through to console.error", async () => {
    ...
    try {
      ...
      expect(String(calls[0]![0])).toMatch(/audit/i);
    }
    finally {
      console.error = orig;
      db = await createDb(resolve(dir, "app.db"));
...
describe("audit()", () => {
  ...
  test("returns undefined and logs when the underlying insert throws", async () => {
...
// Restore the original mock module state after this file's tests run so
// later test files (which may also import audit.service) start clean.
afterEach(() => {
  mock.restore();
});
