...
import { isInitialized, setDbError, setDek, setInitialized } from "./state";
 ...
 */
export async function initEncryption(dbPath: string, publicKeyHex: string, kdfSalt?: string): Promise<{ dekVersion: number }> {
  if (isInitialized()) {
 ...
 */
export async function unlockSystem(dbPath: string, dekHex: string): Promise<void> {
  if (!existsSync(dbPath)) {
 ...
 * 4. Replace old file
 */
export async function rotateDek(dbPath: string, currentDb: AppDatabase, currentDek: string): Promise<{ dekVersion: number }> {
  ...
  try {
    ...
    await reopenWithDek(currentDek);
    throw err;
  }
