...
import { beginOperation, consumeChallenge, createChallenge, endOperation, getBootstrapToken, getDbError, getStatus, isInitialized, isUnlocked, setDbError } from "./state";
...
/** Loopback peer detection for the dev-only bootstrap-token surface. */
function isLoopbackPeer(c: Context<AppEnv>): boolean {
  const addr = c.env?.IP?.address;
 ...
 */
async function decryptDekFromChallenge(challengeId: string, encryptedDekHex: string): Promise<string> {
  const ephPrivKey = consumeChallenge(challengeId);
  if (!ephPrivKey) {
...
export function encryptionStatusRoute() {
  ...
  router.get("/encryption/status", (c) => {
    ...
    // Dev convenience: surface the bootstrap token while uninitialized so the
    // setup page can render it inline instead of asking the operator to scrape
    // stderr. Strict gating — only on a development build, only when the
    // server is listening on a loopback host, and only to a loopback peer.
    const isDevHost = config.HOST === "127.0.0.1" || config.HOST === "localhost";
    const dev
...
export function encryptionPublicRoutes() {
  ...
  router.post("/encryption/unlock-challenge", (c) => {
    const status = getStatus();
    if (status !== "locked") {
    ...
    try {
      challenge = createChallenge(rateLimitKey(c));
    }
    catch (err) {
  ...
  router.post("/encryption/init", async (c) => {
    if (isInitialized()) {
    ...
    const config = c.get("config");
    const body = initSchema.parse(await c.req.json());
    ...
    try {
      const result = await initEncryption(config.DB_PATH, body.publicKey, body.kdfSalt);
  ...
  router.post("/encryption/unlock", async (c) => {
    const status = getStatus();
    ...
    try {
      const dekHex = await decryptDekFromChallenge(body.challengeId, body.encryptedDek);
      await unlockSystem(config.DB_PATH, dekHex);
...
export function encryptionProtectedRoutes() {
  ...
  router.post("/encryption/challenge", adminRequired, (c) => {
    let challenge: ReturnType<typeof createChallenge>;
    try {
      challenge = createChallenge(rateLimitKey(c));
    }
  ...
  router.get("/encryption/meta", adminRequired, (c) => {
    const config = c.get("config");
    const meta = readEncryptionMeta(config.DB_PATH);
    return c.json({
      success: true,
  ...
  router.post("/encryption/rotate-dek", adminRequired, async (c) => {
    ...
    try {
      ...
      const currentDek = await decryptDekFromChallenge(body.challengeId, body.encryptedDek);
      const result = await rotateDek(config.DB_PATH, db, currentDek);
      return c.json({ success: true, data: result });
    }
    catch (err) {
      ...
      const auditId = await audit(db, failurePayload).catch(() => undefined);
      if (!auditId) {
  ...
  router.post("/encryption/change-master", adminRequired, async (c) => {
    if (!isUnlocked()) {
      throw new AppError("System is locked", 503, "SYSTEM_LOCKED");
    }
    if (!beginOperation()) {
