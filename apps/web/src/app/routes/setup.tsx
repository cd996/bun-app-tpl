...
interface EncryptionStatusResponse {
  ...
  data: {
    ...
    status: string;
    finalized?: boolean;
    dev?: { bootstrapToken: string | null };
  };
}
...
function SetupPage() {
  ...
  useEffect(() => {
    http<EncryptionStatusResponse>("/encryption/status")
      .then((res) => {
        const { initialized, status, finalized, dev } = res.data;
        ...
        // Dev-mode hand-off: the API echoes the bootstrap token in the
        // status payload while uninitialized so the operator does not have
        // to scrape stderr. Production never returns this field.
        const devToken = dev?.bootstrapToken;
        if (devToken)
          setBootstrapToken(devToken);
        setChecking(false);
      })
            ...
            <div>
              ...
              {downloaded && (
                <p className="mt-1 text-xs text-green-600">
                  {t("setup.downloaded", { filename: KEY_FILENAME })}
                </p>
              )}
              {copied && (
