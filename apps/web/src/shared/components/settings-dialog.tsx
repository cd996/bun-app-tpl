...
function ProfileTab() {
  ...
  const profileItems = [
    { icon: UserIcon, label: t("profile.username"), value: user.username },
    { icon: Mail, label: t("profile.email"), value: user.email },
    { icon: Shield, label: t("profile.role"), value: user.role === "admin" ? t("profile.roleAdmin") : t("profile.roleUser") },
    { icon: UsersRound, label: t("profile.groups"), value: user.groups.map(g => g.name).join(", ") || "—" },
  ];
