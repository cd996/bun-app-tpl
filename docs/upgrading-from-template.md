...
| Reference modules | `apps/api/src/modules/{account,audit,backup,encryption,policy,settings,system}/` | `apps/api/src/modules/{document,todo}/` once you start adding business fields, your own new modules under `apps/api/src/modules/<your-module>/` |
...
| Aggregate files | `apps/api/src/db/schema.ts`, `apps/api/src/routes/protected.ts`, `apps/web/src/shared/components/sidebar/registry.ts`, `apps/web/public/locales/{en,zh}/common.json` | Your one-line registry entries inside those files (see [`module-standards.md`](module-standards.md)) |
| Docs | `docs/architecture.md`, `docs/api.md`, `docs/database.md`, `docs/module-standards.md`, `docs/rebranding.md`, `docs/deployment.md`, `docs/operations.md`, `docs/upgrading-from-template.md`, `docs/changelog.md` | `docs/modules/<your-module>.md` |
...
- **`docs/changelog.md`**: upstream adds entries above your old top entry — keep both, with upstream entries above yours under your application's running tally if you keep one.
...
When upstream tags a release, the upstream `docs/changelog.md` `Unreleased` section is closed off into a versioned section. Your fork's changelog is independent — keep your own running list of changes you make on top of the template.

A reasonable convention: tag your fork as `v<upstream-version>+<your-suffix>` (for example, `v0.4.0+acme.7`) so anyone reading your tags can tell which upstream baseline you are sitting on.
