...
| POST | `/api/encryption/rotate-dek` | protected | Admin (operation-locked) | **EXPERIMENTAL** — rotates the DEK and rewrites the libsql key. Currently fails with `SQLITE_IOERR` under busy WAL; prefer `change-master` until the tracked fix lands. |
